function X = CMN_TV(E,y,varargin)
q = 2;
p_f = 2;
p_s = 1;
gamma = 5;
...
V3 = zeros(B, N);
V4 = zeros(R, N);
V5x = zeros(n_row,n_col,R);
V5y = zeros(n_row,n_col,R);
Lambda1 = zeros(R,N);
Lambda2 = zeros(R,N);
Lambda3 = zeros(B,N);
Lambda4 = zeros(R,N);
Lambda5x = zeros(n_row,n_col,R);
Lambda5y = zeros(n_row,n_col,R);
tol = 1e-4;
tol1 = sqrt(N * R) * tol;
tol2 = sqrt(N * R) * tol;
i = 1;
...
FDh = zeros(n_row,n_col);
FDh(1,1) = -1;
FDh(1,end) = 1;
FDh = fft2(FDh);
FDhH = conj(FDh);

FDv = zeros(n_row,n_col);
FDv(1,1) = -1;
FDv(end,1) = 1;
FDv = fft2(FDv);
FDvH = conj(FDv);
